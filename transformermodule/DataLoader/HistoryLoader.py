...
class HistoryLoader(Dataset):
    def __init__(self, token2idx, sequences, max_len, conf=None):
        ...
        for seq in self.sequences:
            ...
            if len(code) != self.max_len or len(mask) != self.max_len or len(gender) != self.max_len or len(age) != self.max_len:
                print("Hello")
    ...
    def __getitem__(self, idx):
        ...
        return th.LongTensor(self.codes[idx]), \
               ...
               th.LongTensor([self.labels[idx]]) if self.conf['task'] == 'category' else th.FloatTensor([self.labels[idx]]), \
               th.LongTensor([idx])
